"""
Application routes - simplified version without database.
"""
...
def login_demo():
    """
    Old login page - Microsoft authentication and simulation.
    """
    messages = []
...
def test_login():
    """
    Test authentication - bypass Microsoft auth for testing purposes.
    Only available when TESTING_MODE is enabled.
    """
    from flask import current_app
    
    if not current_app.config.get('TESTING_MODE', False):
        flash('Test authentication is disabled in production mode.', 'danger')
        return redirect(url_for('main.login_demo'))
    
    # Get test user details from form
    ...
    if not user:
        ...
        user.set_password('test-auth-no-password')
        db.session.add(user)
        db.session.commit()
    
    # Store test info in session
    session['test_user_name'] = name
    ...
    # Run post-login security checks
    after_login_attempt(user, success=True)
    
    flash(f'Test authentication successful for {email}', 'success')
    return redirect(url_for('main.login_demo'))
...
def riskgate_dashboard():
    """
    Main RiskGate Dashboard for Microsoft Entra security scanning.
    Shows scan results for tgaskins user.
    """
    from app.models_new import EntraSignInEvent, EntraSecurityAlert
    
    # Target user for scanning
    target_user = 'tgaskins@peakmadeit.onmicrosoft.com'
    
    # Get impossible logins count for tgaskins from database
    impossible_logins_count = EntraSignInEvent.query.filter_by(
        impossible_travel_detected=True
    ).filter(
        EntraSignInEvent.user_principal_name.like('%tgaskins%')
    ).count()
    
    # Get recent alerts for tgaskins from database
    alerts = EntraSecurityAlert.query.filter(
        EntraSecurityAlert.user_principal_name.like('%tgaskins%')
    ).order_by(
    ...
    for alert in alerts:
        recent_alerts.append({
            ...
            'finding': alert.alert_type.replace('_', ' ').title(),
            'risk_level': alert.severity.title(),
            'details': alert.reason[:100] if len(alert.reason) > 100 else alert.reason,
            'status': alert.status.title()
        })
    
    return render_template(
        'riskgate_dashboard.html',
        impossible_logins_count=impossible_logins_count,
...
def scan_entra():
    """
    Scan Microsoft Entra for anomalies in tgaskins sign-in logs.
    Fetches last 24 hours of sign-in logs and analyzes for impossible travel.
    """
    from flask import flash
    from app.graph_client import GraphClient
    from app.ingest import ingest_signin_event
    from app.models_new import EntraSignInEvent
    
    target_user = 'tgaskins'
    
    try:
        # Initialize Graph API client
        graph_client = GraphClient()
        
        # Fetch sign-in logs for last 24 hours
        current_app.logger.info(f"Fetching sign-in logs for {target_user}...")
        signin_logs = graph_client.fetch_signin_logs(hours_back=24, max_results=100)
        
        if not signin_logs:
            flash('No sign-in logs found. Check Microsoft Graph API permissions.', 'warning')
            return redirect(url_for('main.riskgate_dashboard'))
        
        # Filter for tgaskins user
        tgaskins_logs = [log for log in signin_logs if 'tgaskins' in log.get('userPrincipalName', '').lower()]
        
        if not tgaskins_logs:
            flash(f'No sign-in logs found for {target_user} in last 24 hours.', 'info')
            return redirect(url_for('main.riskgate_dashboard'))
        
        # Ingest and analyze sign-in logs
        processed_count = 0
        new_alerts = 0
        
        for signin_record in tgaskins_logs:
            try:
                # Ingest will automatically trigger risk detection and alert creation
                signin_event = ingest_signin_event(signin_record)
                if signin_event:
                    processed_count += 1
                    if signin_event.impossible_travel_detected:
                        new_alerts += 1
            except Exception as e:
        ...
        # Success message
        flash(f'Scan complete! Processed {processed_count} sign-ins for {target_user}. Found {new_alerts} impossible travel events.', 'success')
        
    except Exception as e:
        current_app.logger.error(f"Scan failed: {e}")
        flash(f'Scan failed: {str(e)}. Check Microsoft Graph API configuration.', 'danger')
...
def report_findings():
    """
    Placeholder route for reporting findings to IT Support.
    
    TODO: Implement Microsoft Graph email integration:
    - Generate report with all open alerts
    ...
    flash('Report generation initiated. This will use Microsoft Graph API to send an email to IT Support with scan findings.', 'info')
